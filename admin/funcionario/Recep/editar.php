...
try{
	...
	$sql="SELECT  * from users  where  id_usu=:id";//consulta con marcador, el marcador es :codigo
	...
	$resultado->execute(array(":id"=>$busca));
		...
		if($administrador=$resultado->fetch(PDO::FETCH_ASSOC)){
			...
			<form action="validareditar.php" method="get">
				...
				<h4 class="titulo1">¿Vas a Actualizar este Recepcionista?</h4>
				<br>
				<table class="principal">
					...
					<tr>
						...
						<td  class="titulo">
							<input type="text" class="in" readonly name="doc" value="<?php echo $administrador['id_usu']?>">
						</td>
					</tr>
					...
					<tr>
						...
						<td class="titulo">
							<input type="text" class="in" name="cla" value="<?php echo $administrador['pass']?>" >
						</td>
					</tr>
					<tr>
						<th class="ti">Rol</th>
							<td class="titulo">
								...
								<?php
								$sql= "SELECT * FROM roles where id_rol =2";
